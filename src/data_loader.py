...
def load_data(args):
    ...
    return train_data, eval_data, test_data, train_user_news, train_news_user, test_user_news, test_news_user, \
           topic_news, news_topic, news_title, news_entity, news_group,test_data_old,test_data_new#, train_clicks, test_clicks


def train_random_neighbor(args, train_user_news, train_news_user, topic_news, news_len):
    ...
    _topic_news = np.zeros([1+len(topic_news),args.news_neighbor], dtype=np.int32)
    for i in range(len(topic_news)):
        n_neighbors = len(topic_news[i])
        if n_neighbors >= args.news_neighbor:
            sampled_indices = np.random.choice(list(range(n_neighbors)), size=args.news_neighbor, replace=False)
        else:
            sampled_indices = np.random.choice(list(range(n_neighbors)), size=args.news_neighbor, replace=True)
        _topic_news[i] = np.array([topic_news[i][k] for k in sampled_indices])

    return user_news, news_user, _topic_news


def test_random_neighbor(args, test_user_news, test_news_user, topic_news, news_len):
